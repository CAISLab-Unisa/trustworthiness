...
trainPred2 = cnn_network.classify(train);
TValid = train.Labels;
VALccuracy = sum(trainPred2 == TValid)/numel(TValid)*100
fprintf(fileID,"%f\t",VALccuracy);

CM = confusionmat(TValid,trainPred2);
dlmwrite(strcat("./results/",dataset_name,"/",net_name,"/",ds_split_prefix,"/cm_training.txt"),CM);

figure
confusionchart(TValid,trainPred2,'ColumnSummary','column-normalized',...
...
trainPred2 = cnn_network.classify(validation);
TValid = validation.Labels;
VALccuracy = sum(trainPred2 == TValid)/numel(TValid)*100
fprintf(fileID,"%f\t",VALccuracy);
...
exportgraphics(f,store_path);
 ...
 trainPred2 = cnn_network.classify(test);
TValid = test.Labels;
  VALccuracy = sum(trainPred2 == TValid)/numel(TValid)*100
fprintf(fileID,"%f\t",VALccuracy);
